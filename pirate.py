...
class Pirate:
    ...
    def cal_manhattan_dist(self, next_pos):
        return abs(next_pos[0] - self.treasure_pos[0]) + abs(next_pos[1] - self.treasure_pos[1])
    ...
    def move_4_directions(self, cur_move):
        ...
        submask_2 = np.logical_and(self.hint_manager.map.map[col, row] != 0, np.array(
            [not np.any(np.equal(p, self.hint_manager.map.mountains).all(1)) for p in pos]))
